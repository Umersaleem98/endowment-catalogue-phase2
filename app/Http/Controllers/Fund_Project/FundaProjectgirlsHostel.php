...
class FundaProjectgirlsHostel extends Controller
{
    public function index($id)
    {
        $projectcategory = ProjectCategory::find($id);
        return view('template.fund_project.girlsHostel.index', compact('projectcategory'));
    }

    public function store(Request $request)
    {
        $BoysHostelCostEstimate = new GirlsHostelCostEstimate;
        ...
        if ($request->hasFile('prove')) {
            ...
            $file->move(public_path('uploads/fundaprojects_payments_girls-proof'), $filename);
            $BoysHostelCostEstimate->prove = 'uploads/fundaprojects_payments_girls-proof/' . $filename;
        }
